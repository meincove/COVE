...
vec3 curlNoise(vec3 p) {
  const float e = 0.1;
  float dx = cnoise(p + vec3(e,0.0,0.0)) - cnoise(p - vec3(e,0.0,0.0));
  float dy = cnoise(p + vec3(0.0,e,0.0)) - cnoise(p - vec3(0.0,e,0.0));
  float dz = cnoise(p + vec3(0.0,0.0,e)) - cnoise(p - vec3(0.0,0.0,e));

  const float divisor = 1.0 / (2.0 * e);
  return normalize(vec3(dy - dz, dz - dx, dx - dy) * divisor);
}
`;
