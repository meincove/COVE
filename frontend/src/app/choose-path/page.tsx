...
export default function ChoosePathPage() {
    ...
    return (
        <div className="relative h-screen w-screen overflow-hidden">
            {/* Animated Background - changes based on hover */}
            <AnimatePresence mode="wait">
                {hoveredPath === 'shop' && (
                    <motion.div
                        ...
                        className="absolute inset-0 bg-gradient-to-br from-pink-50 via-yellow-50 to-blue-50"
                    />
                )}
                {hoveredPath === 'platform' && (
                    <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        transition={{ duration: 0.6 }}
                        className="absolute inset-0 bg-gradient-to-br from-green-50 via-white to-slate-50"
                    />
                )}
                {!hoveredPath && (
                    <motion.div
            ...
            <div className="relative h-full w-full flex items-center justify-center p-4 md:p-8">
                <div className="w-full max-w-7xl">
                    ...
                    <div className="grid md:grid-cols-2 gap-4 md:gap-8 mb-6 md:mb-8">
                        {/* COVE SHOP Card - Pink/Yellow/Blue Theme */}
                        <motion.div
                ...
                ${hoveredPath === 'shop'
                                    ? 'border-pink-400 shadow-2xl shadow-pink-500/20 scale-[1.02]'
                                    : 'border-slate-200 hover:border-pink-300 shadow-lg'
                                }
              `}
                        >
                            <div className="p-6 md:p-10">
                                {/* Badge */}
                                <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-pink-50 to-yellow-50 rounded-full mb-4 md:mb-6">
                                    <span className="text-2xl">🛍️</span>
                                    <span className="text-sm font-semibold bg-gradient-to-r from-pink-600 to-yellow-600 bg-clip-text text-transparent">
                                        For Shoppers
                                    </span>
                                </div>

                                {/* Title */}
                                <h2 className="text-2xl md:text-4xl font-bold bg-gradient-to-r from-pink-600 via-yellow-500 to-blue-600 bg-clip-text text-transparent mb-2 md:mb-3">
                                    COVE SHOP
                                </h2>
                                ...
                                {/* Visual Placeholder */}
                                <div className="relative h-32 md:h-48 mb-6 md:mb-8 rounded-2xl bg-gradient-to-br from-pink-100 via-yellow-100 to-blue-100 flex items-center justify-center overflow-hidden">
                    ...
                    ${hoveredPath === 'shop'
                                            ? 'bg-gradient-to-r from-pink-500 via-yellow-500 to-blue-500 text-white shadow-lg shadow-pink-500/30'
                  ...
                  `}
                            ...
                            {hoveredPath === 'shop' && (
                                <motion.div
                                    initial={{ opacity: 0 }}
                                    animate={{ opacity: 1 }}
                                    className="absolute inset-0 bg-gradient-to-br from-pink-500/5 via-yellow-500/5 to-blue-500/5 pointer-events-none"
                                />
                            )}
                        </motion.div>

                        {/* COVE PLATFORM Card - Green/Black/White Theme */}
                        <motion.div
                            initial={{ opacity: 0, x: 50 }}
                            animate={{ opacity: 1, x: 0 }}
                            transition={{ duration: 0.6, delay: 0.2 }}
